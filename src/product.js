...
var PORT = 5000;
var products = JSON.parse(fs.readFileSync(path.join(__dirname, 'Product.json'), 'utf-8'));
...
app.post('/api/products', function (req, res) {
    ...
    if (req.body.color !== undefined) {
        if (typeof req.body.color !== 'string')
            return res.status(404).send('you must send color property as string');
    }
    if (req.body.description !== undefined) {
        if (typeof req.body.description !== 'string')
            return res.status(404).send('you must send description property as string');
    }
    if (req.body.stockCount !== undefined) {
        if (typeof req.body.stockCount !== 'number')
            return res.status(404).send('you must send stockCount property as number');
    }
    if (req.body.expirationDate !== undefined) {
        if (isNaN(new Date(req.body.expirationDate).getTime()))
            return res.status(404).send('you must send valid expirationDate property as Date like this 1/7/2021');
    }
    for (var i = 0; i < products.length; i++) {
        if (products[i]['code'] == req.body.code)
            return res.status(404).send('you must send unique code');
    }
    var product = {
